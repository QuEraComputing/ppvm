...
relative to `ppvm-2`, and the size of the propagated operator — which is the
...
# ΔE 9.2, worst normal-vision ΔE 27.5). Adding `ppvm-1` deliberately did not
# repaint the other four: colour follows the entity, so a run with a subset of
# `--libs` keeps every survivor's hue. Magenta and aqua sit below 3:1 on the
# light surface, which obligates relief — `results.csv`, the driver's summary
# table, and the per-series markers are it.
BASELINE = "ppvm-2"
SERIES = {
    "ppvm-1": ("ppvm-1 (legacy)", "#e87ba4", "v"),
    "ppvm-2": ("ppvm-2 (this repo)", "#2a78d6", "o"),
...
def main() -> None:
    ...
    caption = (
        ...
        "term-for-term before timing, but pauli-prop's atol prunes at "
        "branch-creation time rather than after accumulation, so at this "
        "truncation it carries a different support — compare the workload panel "
        "before reading its ratio. Per-point numbers in results.csv."
    )
    if args.title:
        fig.suptitle(
