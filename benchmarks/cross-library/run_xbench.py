...
through `ppvm`, PauliPropagation.jl, PauliStrings.jl, Qiskit's `pauli-prop` and
monoprop, all from one parameter contract, and writes one tidy CSV.
...
against `ppvm`. A cross-library benchmark whose engines are quietly computing
different things is worse than no benchmark, and every bug this check caught
while the harness was being written produced plausible-looking numbers that
agreed on the observable to nine digits — a reversed circuit in the `pauli-prop`
runner, duplicate Paulis collapsing in its readout, and monoprop's `exp(+iθH)`
sign convention.

    uv run --no-project python3 run_xbench.py --help
"""
...
RUNNERS = {
    ...
    "pauli-prop": Runner(
        ...
        [
            ...
            "pauli-prop",
            "python3",
            _here("xbench_qiskit.py"),
        ],
        "uv",
    ),
    # monoprop takes one serial partition per physical core when left alone, so
    # the cap is part of the invocation, not an optional extra. The runner
    # re-asserts it from its own CPU/wall ratio and fails if it did not land.
    "monoprop": Runner(
        "monoprop",
        [
            "uv",
            ...
            "monoprop",
            "python3",
            _here("xbench_monoprop.py"),
        ],
        "uv",
        {"monoprop_NUM_THREADS": "1", "monoprop_PARTITIONS": "off"},
    ),
}
...
def summarize(rows: list[dict[str, str]]) -> None:
    """Print a time table plus the speedup of every engine relative to `ppvm`."""
    ...
    for model, per_n in by_model.items():
        ...
        for lib, notes in drift.items():
            print(f"  note: {lib} support vs {BASELINE} — {', '.join(notes)}")
