...
pub fn bench(c: &mut Criterion) {
    let mut g = c.benchmark_group("word_surface/lossy/clifford_present/256");
    ...
    // Loss-guard behavior is performance-relevant and differs from a present
    // gate, so benchmark one representative lost-site path explicitly.
    let mut lost = c.benchmark_group("word_surface/lossy/clifford_lost_guard/256");
    let q = 124; // lossy pattern index 124 is L
    let mut nc = new.clone();
    let mut oc = old.clone();
    nc.h(q);
    oc.h(q);
    assert_eq!(nc.to_string(), oc.to_string());
    lost.bench_function("new/h_lost_noop", |b| {
        b.iter_batched(
            || new.clone(),
            |mut w| {
                w.h(black_box(q));
    ...
    lost.bench_function("old/h_lost_noop", |b| {
        b.iter_batched(
            || old.clone(),
            |mut w| {
                w.h(black_box(q));
