...
/// Dipolar-pair geometry: the `(a, b)` site pairs, each pair's coupling
/// magnitude, and its unit separation vector.
type Geometry = (Vec<(usize, usize)>, Vec<f64>, Vec<[f64; 3]>);

/// A Kossakowski dissipator as handed to `LindbladSpec::add_kossakowski`:
/// the operators `A_n` as Pauli lincombs, and the pair matrix `K`.
type KossakowskiModel = (Vec<Vec<(String, Complex<f64>)>>, Vec<Vec<Complex<f64>>>);
// spatial harmonics m = -2..2
...
/// pickles decompose.
fn kossakowski_model(n: usize) -> KossakowskiModel {
