...
fn chain_couplings(n: usize) -> (Vec<Vec<f64>>, Vec<Vec<f64>>) {
    ...
    for (a, (j_row, gam_row)) in j.iter_mut().zip(gam.iter_mut()).enumerate() {
        for b in 0..n {
            if a == b {
                gam_row[b] = G0;
                continue;
            ...
            j_row[b] = -3.0 * PI * G0 / k0 * g.re;
            gam_row[b] = 6.0 * PI * G0 / k0 * g.im;
        }
    }
    (j, gam)
}
...
fn hamiltonian_terms(n: usize, j: &[Vec<f64>]) -> Vec<(String, f64)> {
    let mut h = Vec::new();
    for (a, j_row) in j.iter().enumerate() {
        for (b, &j_ab) in j_row.iter().enumerate().skip(a + 1) {
            if j_ab.abs() > 1e-14 {
                h.push((pstr(n, &[(a, 'X'), (b, 'X')]), j_ab / 2.0));
                h.push((pstr(n, &[(a, 'Y'), (b, 'Y')]), j_ab / 2.0));
            }
        }
    }
    h
...
fn observable(n: usize, gam: &[Vec<f64>]) -> (Vec<Word>, Vec<f64>) {
    ...
    for (a, gam_row) in gam.iter().enumerate() {
        push(pstr(n, &[(a, 'Z')]), G0 / 2.0);
        for (b, &g_ab) in gam_row.iter().enumerate().skip(a + 1) {
            push(pstr(n, &[(a, 'X'), (b, 'X')]), g_ab / 2.0);
            push(pstr(n, &[(a, 'Y'), (b, 'Y')]), g_ab / 2.0);
        }
    }
    (basis, coeffs)
}
