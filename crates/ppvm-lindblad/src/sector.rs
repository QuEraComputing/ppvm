...
//! shift counter `cnt_q`, and accumulate `χ_k(g_{cnt_q}) · v · c_r`
//! (where `v` is the matrix element of `L*` between input rep `r` and
//! output `q`). [`Sector::canonicalize_phase`] is that step.
//!
...
impl<'a> Sector<'a> {
    ...
    /// element that maps `q` to `r_q`. The phase weights the matrix
    /// element of `L*` when it is accumulated onto `r_q`.
    #[inline]
    pub fn canonicalize_phase(&self, q: &Word) -> (Word, Complex<f64>) {
        let (rep, counter) = self.group.canonicalize_with_shift(q);
