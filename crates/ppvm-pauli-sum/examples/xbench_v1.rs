...
//! The **v1** (pre-`traits-2`) side of the cross-library Pauli-propagation
//! benchmark — the legacy `ppvm-pauli-sum` engine running the identical circuit
//! its `ppvm-pauli-sum-2` successor runs in
//! `crates/ppvm-pauli-sum-2/examples/xbench.rs`.
//!
//! Same two models, same gate order, same `θ = 2·c·dt`, same
//! truncate-after-every-gate rule, same environment contract, same CSV. See
//! `benchmarks/cross-library/README.md` for the shared spec.
//!
//! Configuration is matched to the `-2` runner deliberately: a `HashMap`-backed
//! config with `[u8; N]` storage and `FxHasher` (`config::fxhash::Byte`) against
//! the `-2` `HashMapStore<PauliWord<[u8; N]>, f64>`, and
//! `CoefficientThreshold` against `CoefficientThreshold`. Comparing the two
//! versions on different storage backends would measure the backend, not the
//! refactor.
//!
//! ```bash
//! MODEL=tfim QUBITS=8,16,24 STEPS=10 DT=0.1 JCOUP=1.0 HFIELD=1.0 ATOL=1e-6 \
//!   cargo run --release -p ppvm-pauli-sum --example xbench_v1
//! ```
...
///
/// v1's `PauliPattern` has no `zero_state()` constructor, so the diagonal
/// contraction is spelled out: `⟨0|Z|0⟩ = 1` and `⟨0|X|0⟩ = ⟨0|Y|0⟩ = 0`, so
/// exactly the X-free terms survive and each contributes its own coefficient.
...
fn main() {
    ...
    eprintln!(
        "ppvm-1 {}: steps={} dt={} J={} h={} atol={:e} iters={}",
        p.model.name(),
        p.steps,
        p.dt,
        p.j,
