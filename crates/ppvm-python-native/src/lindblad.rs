...
impl LindbladSpec {
    ...
    ///
    /// Same algorithm as `pc_step` but with complex `coeffs`. Use when
    /// tracking a momentum-projected operator (single sector of the
    /// translation group).
    ///
    /// `momentum`, when provided alongside `group`, triggers a
    /// pre-step sector check: the input `(basis, coeffs)` must already
    /// be a momentum-`momentum` eigenstate under `group`, otherwise an
    /// error is raised and no evolution is performed. `momentum` is a
    /// length-`group.n_generators` array of integer mode indices.
    ///
    /// Symmetry merging is NOT applied automatically here — call
    /// `canonicalize_basis_arr_complex(basis, coeffs, group, momentum)`
    /// at snapshot points to project to orbit-rep form.
    #[pyo3(signature = (
    ...
    fn pc_step_complex<'py>(
        ...
        let momentum_owned = match momentum_vec {
            Some(Ok(v)) => Some(v),
            Some(Err(e)) => return Err(PyErr::from(e)),
            None => None,
        };
        let sym_core = group.map(|g| g.core());
