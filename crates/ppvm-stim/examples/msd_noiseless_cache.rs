...
//! Noiseless MSD-shaped STIM sampling with and without trajectory caching.
//!
//! This is the no-noise counterpart to `msd_noisy_cache.rs`: it keeps the final
//! batched `M` readout, but removes every injected loss/depolarizing channel.
//! That leaves one stochastic boundary at the final readout, so cached sampling
//! can reuse the whole deterministic circuit prefix.
//!
//! ```text
//! cargo run --release -p ppvm-stim --example msd_noiseless_cache -- 4096 32768
//! ```
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let circuit = msd_noiseless_stim_string();
    let prog = parse_extended(&circuit)?;

    println!(
        "noiseless MSD STIM: {N_QUBITS} qubits, {} measurements, {shots} shots",
...
fn msd_noiseless_stim_string() -> String {
