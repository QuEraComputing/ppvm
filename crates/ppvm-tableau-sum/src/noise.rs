...
use ppvm_traits::traits::{
    ...
    ResetLossChannel, Targets, TwoQubitPauliError,
};
    ...
    I: Debug,
{
    fn pauli_error(&mut self, targets: impl Targets, p: [<T as Config>::Coeff; 3]) {
        for addr0 in targets.each() {
            ...
            let mut branches =
                Vec::<(GeneralizedTableau<T, I, C>, T::Coeff, u64, u64)>::with_capacity(
                    3 * self.entries.len(),
                );
...
where
    ...
    I: Debug,
{
    fn depolarize2(&mut self, targets: impl Targets, p: <T as Config>::Coeff) {
        for (addr0, addr1) in targets.pairs() {
