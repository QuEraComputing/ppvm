...
One row remains open, `perf.sym.fixture` (the symbolic propagation family
measures a 131072-bucket control-byte sweep over a 2-term support, and drifts
10%+ run to run on the **old** side). `perf.cy` — the 1.11× single-gate sum `CY`
— is now **closed** at 0.895×. Another, `perf.trotter.native`, was chased
...
| perf.cy | perf-drift | med | impl | **closed — 1.107× → 0.895×** | The earlier pass diagnosed the mechanism right and verified the wrong thing: it checked that `cy_toggles` had no call sites, not that the re-key **closure** had been inlined. It had not — `nm` still showed the `rekey_owned::{{closure}}` symbol with one `bl` per term from `CY`'s loop. A 32-byte `PauliWord` exceeds the AArch64 register-return limit, so out-of-line means two plane copies to the stack, four byte read-modify-writes instead of register toggles, a 16-byte reload overlapping those stores (store-to-load-forwarding stall, on the digest's dependency chain) and an indirect sret — ~2 ns/term, exactly the gap. That also dissolves the paradox: the *algorithmic* memory work really is lower, the ABI adds it back. Why `CY` alone: every gate is `#[inline(always)]`, so the re-key loop is cloned into each caller, the closure gets several call sites, LLVM's last-call bonus never applies, and `CY`'s body (one toggle more, plus the eager digest) sits just over the cost threshold its siblings sit under. **Fix:** `cy` becomes `#[inline(never)]` — one attribute, gate body byte-identical. One copy of the loop → one call → LLVM folds it in; one extra call per *gate* instead of a stack round-trip per *term*. `cy` 0.895× (0.890–0.896), `zcy_alias` 0.889×, `clifford_batch/cy` 0.785×, all 4 launches; layout control agrees (0.901×); siblings unmoved. **Lesson:** `#[inline(always)]` on a gate owning a per-term closure can *prevent* that closure being inlined by multiplying the call sites the cost model sees — verify with `nm | grep closure`, not by reading attributes. |
