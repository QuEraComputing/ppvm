...
function benchmark_suite()
    ...
    group["rx"] = @benchmarkable propagate!($rx, state; min_abs_coeff = 1e-10) setup = (state = copy($initial_state); cache_state = copy($tmp_state))
    group["ry"] = @benchmarkable propagate!($ry, state; min_abs_coeff = 1e-10) setup = (state = copy($initial_state); cache_state = copy($tmp_state))
    group["rz"] = @benchmarkable propagate!($rz, state; min_abs_coeff = 1e-10) setup = (state = copy($initial_state); cache_state = copy($tmp_state))

    group["rxx"] = @benchmarkable propagate!($rxx, state; min_abs_coeff = 1e-10) setup = (state = copy($initial_state); cache_state = copy($tmp_state))
    group["ryy"] = @benchmarkable propagate!($ryy, state; min_abs_coeff = 1e-10) setup = (state = copy($initial_state); cache_state = copy($tmp_state))
    group["rzz"] = @benchmarkable propagate!($rzz, state; min_abs_coeff = 1e-10) setup = (state = copy($initial_state); cache_state = copy($tmp_state))
