...
class PauliSum(
    ...
    Example:
        ...
        For z-magnetization-conserving (XY / Heisenberg-style) dynamics, use
        the U(1) helpers — `exchange`, `xyzz`, and `apply_u1_trotter_step` —
        which propagate the appropriate XX+YY and ZZ generators in
        Heisenberg-picture order without manual rxx/ryy/rzz composition.
    """
    ...
    def apply_u1_trotter_step(
        self,
        edges: "Sequence[tuple[int, int] | tuple[int, int, float]]",
        theta_xy: "float | Sequence[float]",
        theta_zz: "float | Sequence[float] | None" = None,
        fields_z: "float | Sequence[float] | None" = None,
    ) -> None:
        """Apply one Trotter slice of a U(1)-symmetric (z-magnetization-conserving)
        Hamiltonian.

        For each edge `(i, j)` (or `(i, j, J)`) in `edges` this applies the
        Heisenberg-picture image of

        ```math
        e^{-i \\frac{\\theta_{xy}}{2} (X_i X_j + Y_i Y_j)}
        \\cdot e^{-i \\frac{\\theta_{zz}}{2} Z_i Z_j}
        ```

        followed by per-site `rz(\\theta_h)` rotations for any non-zero entries
        in `fields_z`. The combined unitary commutes with `\\sum_k Z_k`, so any
        observable in the total-Z sector stays in that sector. The angle
        convention matches the rest of PauliSum: each rotation is
        `exp(-i θ/2 G)`.

        Args:
            edges: A sequence of `(i, j)` pairs or `(i, j, J_ij)` triples
                identifying the two-site interactions. The optional third
                entry scales the XX+YY and ZZ angles for that edge.
            theta_xy: The base `XX + YY` angle for every edge, or a per-edge
                sequence of the same length as `edges`. Set to zero to skip
                the exchange step.
            theta_zz: Optional `ZZ` angle. Same shape rules as `theta_xy`.
                If `None` or zero, no `ZZ` rotation is applied.
            fields_z: Optional per-site Z field. Either a scalar applied to
                every site, a sequence of length `n_qubits`, or `None`. If
                `None` or zero on a site, the `Z` rotation for that site is
                skipped.

        Notes:
            Gates are applied in the order:
                exchange(edge), rzz(edge), then rz(site) for each site.
        ...
        Conservation under truncation:
            Every gate emitted by this helper commutes with the total Z
            magnetization :math:`\\sum_k Z_k`, so an observable built from
            `{I, Z}`-only Pauli strings — :math:`\\sum_i Z_i`,
            :math:`\\sum_{i<j} Z_i Z_j` (the sum over *all* pairs, not
            just nearest neighbours), or any polynomial in `Z_i` — is
            conserved through repeated calls to this helper up to
            per-gate floating-point precision (~1e-15 per gate, growing
            linearly with the number of gates). Standard truncation
            settings preserve this guarantee: pick `min_abs_coeff` well
            below the conserved coefficient magnitude (usually 1) and
            well above the per-gate ε floor (≈ 1e-12 for short circuits,
            looser for long ones). Setting `min_abs_coeff` close to 1, or
            running enough gates for the accumulated drift to approach
            the cutoff, can break this guarantee. For more general
            U(1)-conserving Hermitian observables (spin currents,
            higher-weight correlators), conjugate-paired Pauli strings
            carry coefficients of equal magnitude, so symmetric truncation
            keeps the observable inside the U(1) sub-algebra even when
            individual coefficient values drift.

        Example:
            ```python
            ps = PauliSum.new(4, ["Z0", "Z1", "Z2", "Z3"])
            ps.apply_u1_trotter_step(
                edges=[(0, 1), (1, 2), (2, 3)],
                theta_xy=0.2,
                theta_zz=0.05,
                fields_z=[0.1, 0.0, -0.1, 0.0],
            )
            ```
        """
        n_edges = len(edges)
        ...
        def _resolve_per_edge(value):
            if value is None:
                return [0.0] * n_edges
            if isinstance(value, (int, float)):
                return [float(value)] * n_edges
            value = list(value)
            if len(value) != n_edges:
                raise ValueError(f"Expected {n_edges} entries for per-edge angle, got {len(value)}")
            return [float(v) for v in value]

        theta_xy_list = _resolve_per_edge(theta_xy)
        theta_zz_list = _resolve_per_edge(theta_zz)

        if fields_z is None:
            fields_list: list[float] = [0.0] * n
        elif isinstance(fields_z, (int, float)):
            fields_list = [float(fields_z)] * n
        else:
            fields_list = [float(v) for v in fields_z]
            if len(fields_list) != n:
                raise ValueError(
                    f"fields_z must have length n_qubits ({n}), got {len(fields_list)}"
                )
        ...
        for k, edge in enumerate(edges):
            if len(edge) == 2:
                ...
                raise ValueError(f"Edge {edge!r} must be a 2- or 3-tuple (i, j[, coupling])")

            theta_xy_eff = theta_xy_list[k] * coupling
            theta_zz_eff = theta_zz_list[k] * coupling
            if theta_xy_eff != 0.0 and theta_zz_eff != 0.0:
                self.xyzz(i, j, theta_xy_eff, theta_zz_eff)
            elif theta_xy_eff != 0.0:
                self.exchange(i, j, theta_xy_eff)
            elif theta_zz_eff != 0.0:
                self.rzz(i, j, theta_zz_eff)

        for site, h in enumerate(fields_list):
            if h != 0.0:
                self.rz(site, h)
