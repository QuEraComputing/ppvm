...
projector `canonicalize_basis_arr_complex`:

    c_rep = coeff of the representative word itself,

on every orbit, whether or not it has a non-trivial stabilizer.
...
where I is stabilized by the whole group) carry the correct weight. An
orbit contributes ``|orbit| · c_rep`` to a translation-invariant {I,Z}
sum, so the emission rate in rep space is ``R = Σ |orbit(rep)| · c_rep``
over {I,Z} reps — see `cyclic_orbit_size` below.
...
def cyclic_orbit_size(word: str) -> int:
    """Number of distinct cyclic rotations of `word` — its orbit size under
    `TranslationGroup.chain_1d`. Words fixed by a non-trivial shift (e.g.
    the identity, or `ZIZI`) have an orbit smaller than `|G|`."""
    return len({word[i:] + word[:i] for i in range(len(word))})


def orbit_rate_trace(lind, obs, n, dt, steps, group, mom, max_basis=BIG, admit=None):
    """R(t) from the orbit-rep evolution: the {I,Z} sum over all real-space
    words, reassembled as `Σ_reps |orbit(rep)| · c_rep`."""
    strings = list(obs)
    basis0 = _basis_to_codes(strings, n)
    coeff0 = np.array([obs[s] for s in strings])
    br, cr = canonicalize_basis_arr_complex(basis0, coeff0.astype(np.complex128), group, mom)
    ...
    for k in range(steps + 1):
        ...
        sizes = np.array([cyclic_orbit_size(w) for w in _codes_to_basis(br[iz])])
        R[k] = (sizes * cr[iz]).sum().real
...
def test_identity_bookkeeping_closed_form():
    ...
    Rep-space coefficients are member coefficients on every orbit,
    stabilized or not, so c_Z = e^{-Γ0 t} and c_I = coeff_I — the identity
    orbit has a single member and carries its full weight. This pins the
    non-unital bookkeeping across orbits of different sizes."""
    ...
    expected_i = -n * (1 - np.exp(-t))
    assert abs(c_i - expected_i) < 1e-9, f"c_I = {c_i} vs coeff_I = {expected_i}"
