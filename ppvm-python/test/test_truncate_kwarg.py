...
def test_generalized_tableau_gates_reject_truncate_kwarg():
    ...
    for call in (
        ...
        lambda: tab.rx(0, 0.3, truncate=False),  # ty: ignore[unknown-argument]
        lambda: tab.rxx(0, 1, 0.2, truncate=False),  # ty: ignore[unknown-argument]
        lambda: tab.depolarize(0, 1e-3, truncate=False),  # ty: ignore[unknown-argument]
    ):
